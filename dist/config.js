 ...
 */
let PdogsData; // 定義全域變數

async function loadJson() {
  try {
    const response = await fetch('contest_data.json');
    PdogsData = await response.json();
    console.log('PDOGS JSON 資料已載入');
  } catch (error) {
    console.error('載入 JSON 時發生錯誤:', error);
  }
}

loadJson(); // 呼叫函式載入 JSON
...
async function initConfig(){
    ...
    config =
    {
        ...
        auth_token: PdogsData['auth_token'],
         ...
         */
        apiBase : feed_server_path + "/" + PdogsData['scoreboard_id'],
